...
title: alipay_cny
excerpt: >-
  Alipay is a digital wallet that stores all your payment info electronically. A
  dominant mobile and online payment platform, revolutionizing how people
  transact in China.
deprecated: false
hidden: false
metadata:
...
Tazapay uses a `payin` object to represent your intent to collect a payment from your customer. The payin object tracks state changes from transaction creation to payment completion via alipay.\
Create a payin on your server with an amount, invoice\_currency `CNY` and a transaction\_description using the [create payin API](https://docs.tazapay.com/reference/create-payin)
