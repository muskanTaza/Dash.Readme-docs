...
1. [Create User](ref:create-user-api) for adding the beneficiary user to the system (requires email ID, country, ind\_bus\_type, and business name or first name/last name as per the ind\_bus\_type)
...
1. **seller\_id:** beneficiary’s user ID
2. **buyer\_id:** either use
3. **Marketplace/Platform’s UUID**
4. Transacting party acting as the marketplace/platform’s sender
5. **invoice\_amount:** actual amount to be paid out to the beneficiary
6. **invoice\_currency:** the currency of the payout (can be the beneficiary’s local currency or USD)
7. **txn\_description:** description of the underlying payout (why is this payout being made)
8. **transaction\_category:** this should be set to ‘payout’
9. **transaction\_source:** (Optional) can be hard-coded to API
